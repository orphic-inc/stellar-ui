...
| `field`   | a form control (input / textarea / select)           | `--st-raised`, `--st-border`, `--st-text` (placeholder → `--st-text-faint`; checkbox & radio: see ADR-0006) |
...
> **Applied in WS6 (release browser):** `ReleaseBrowsePage` migrated with **no new Roles, Parts, or tokens** — proof the WS5 vocabulary already covers a whole surface end-to-end (the release listing, item 2 of the §7 order). The filter form is a `panel` of `field` inputs with `meta` labels and `control`(`-primary`) buttons; the results are the `grid`/`colhead`/`row` table variant with `title`/`meta`(`-em`)/`chip` cells; pagination is `control`(`-primary` for the current page). First surface to carry checkboxes/radios; no checkbox Role is needed. _(Corrected in #368: this note first said they took their `accent-color` tint from `field`, whose box rules would be inert on an `appearance:auto` control. `@tailwindcss/forms` had already set `appearance: none`, so no checked state showed. An element rule in `src/index.css` now draws every checkbox and radio natively. See the ADR-0006 amendment.)_ The `edition-*`/`coverart-*` Parts stay unwired here: rip-quality is in no readable schema (§5), and a per-row cover thumbnail is not the mosaic Part's job.
...
[ui#196](https://github.com/orphic-inc/stellar-ui/issues/196). Sublime keeps
`isDefault` and its picker entry; it is simply a row with no delivery target.
...
This section's original optimism — that editions could lazy-load "from the release-workbench endpoint, no API change" — was **wrong** (a WS2 finding), and the required API change has since **landed** (stellar-api #129). The contributor and cover-mosaic enrichments never needed an API; the **edition disclosure did**, and now has one:

- **Rip-quality now has a read path (resolved).** The gap was real — `bitrate` / `media` / `hasLog` / `hasCue` / `isScene` were absent from every UI-readable response (`getReleaseById` → `ReleaseContribution` is lean; `/contributions` → `Contribution` omitted quality; `/communities/{id}/releases/{id}/contributions` was `get?: never`). stellar-api #129 added a release-scoped **`GET /communities/{id}/releases/{id}/contributions`** returning each contribution with nested `releaseFile` (`bitrate`/`hasLog`/`hasCue`/`isScene`) and full `edition` identity (media + year/label/cat#/title/isRemaster). The `edition-*` Part is now **wired** via the shared `EditionStack` component — on the release page (actions inline) and the collage row-expand disclosure (read-only). Vendor the shape with `npm run api:sync`.
...
- **WS2 — Collage pilot (UI-only).** `CollageDetail.tsx` renders from Roles + Parts + tokens (no inline paint, no regression), adding the contributor power-law block (`bar` Role) and the weighted 2×2 cover mosaic. _Done._ The edition disclosure it deferred is now **wired** (the `edition-*` Part via `EditionStack`, on `CollageDetail` row-expand + `ReleasePage`), once stellar-api #129 exposed rip-quality on a read (§5).
