...
2. **Precedence is single-winner, not a stack (ADR-0024 §4).** The slot resolves to exactly one `href`: `externalStylesheet` (if injectable) → Registry (`activeAuthorStylesheetId`) → `siteAppearance` built-in → Sublime (no link) — _the final arm was superseded in 2026-07-20; see the amendment below._ The API enforces Personal XOR Registry, so at most one of the first two branches is ever populated; if both somehow arrive, external wins and there is still only one `<link id="stellar-theme">`. `siteAppearance` is a **separate axis** (the built-in fallback), not one of the two explicit-slot arms.

   **Amendment (2026-07-20, [#196](https://github.com/orphic-inc/stellar-ui/issues/196)).** The chain no longer ends in a distinct "→ Sublime (no link)" branch. Sublime resolves through the `siteAppearance` built-in branch like any other registry row and links nothing because its row's `cssUrl` is `null` (stellar-api [#377](https://github.com/orphic-inc/stellar-api/pull/377) / ADR-0024 §3), rather than because the injector compares the name against `'sublime'`. Two consequences beyond deleting a magic string: an operator who repoints Sublime at a real delivery target now gets it honoured, and a Sublime user's pre-applied `<link>` is no longer torn down before the stylesheets query resolves (the name comparison short-circuited ahead of the `undefined` guard). It also lets the API's registry partition stay total — a client that recognises the name forces an API carve-out, and a carve-out is where the next unreachable theme hides.
