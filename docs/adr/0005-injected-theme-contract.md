...
## Addendum (2026-07-20): the contract's two halves are co-located in `src/`, and the token list is now duplicated across repos

Bookkeeping on the 2026-06-27 addendum. Does **not** change the contract shape.

**Decision — the file moved.** `global.css` now lives at `src/global.css`, beside `src/index.scss`. It was at `src/stylesheets/common/global.css`, which implied it was a theme; it is not one, it is half the contract. After ui#168 emptied `src/stylesheets/` of everything but the blocked `postmod`, keeping the contract filed under a directory of themes actively misled. The two halves now sit together: primitives in `index.scss`, derived tokens and hooks in `global.css`. Every reference in this ADR's earlier sections to the old path means this file. Rejected: `src/styles/global.css`, which splits the contract across two locations unless `index.scss` moves too — and moving that drags the webpack/Tailwind entry point along for no gain.

**Consequence — the primitive list is now duplicated across repos, and nothing detects divergence.** ui#168 left stellar-api as the only place any delivered theme's token conformance is checked; this repo's `themes.tokens.test.ts` pins its own contract (`index.scss`), not the themes. So the api's `REQUIRED_ST_PRIMITIVES` and this repo's `PRIMITIVE_TOKENS` are independent copies of one 21-token contract, byte-identical as of this date. The failure mode: the api list is weakened, a theme ships missing a token, and ui surfaces reading it fall back to Sublime values silently instead of failing. Accepted, not repaired — re-adding a ui-side theme check would restore exactly the duplication ui#168 removed, since that check was accidental redundancy rather than a designed backstop. See `docs/theming.md` §4.1 for the rejected alternatives (cross-repo CI check; single-sourcing through the type-generation pipeline).

**Amendment — the primitives file is now `src/index.css`.** The extension changed; the contract did not. Every reference above to `src/index.scss` means this file. It never contained Sass syntax — no variables, mixins, `@use`, `@extend` or control directives — only plain CSS and Tailwind v4 at-rules, so the `.scss` extension bought nothing and cost something: webpack applied `sass-loader` to it (and, because the rule matched `.css` too, to `global.css` as well), which meant Dart Sass parsed and re-emitted both halves of the contract. That re-emission was not lossless — it simplified `clamp(0%, calc(var(--st-w, 0) * 1%), 100%)` down to `clamp(0%,var(--st-w,0)*1%,100%)` and rewrote `@media (prefers-reduced-motion: reduce)` to `@media(prefers-reduced-motion:reduce)`. Sass also emitted a deprecation warning on `@import "tailwindcss"`, reading Tailwind's own required directive as a Sass import due for removal in Dart Sass 3.0. With `sass-loader` dropped from the chain the authored CSS now reaches the browser byte-for-byte, and `sass-loader` and `normalize-scss` are removed as dependencies. Rejected: keeping the `.scss` name with Sass no longer processing it, which would have avoided touching this ADR at the cost of an extension that lies about the pipeline.
