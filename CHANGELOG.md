...
- **Going to or from the home page no longer flashes the default theme** (#161). Home had a layout instance of its own, so every crossing between it and another private page remounted the layout. Since #379 a remount removes the theme link and re-creates it, so a couple of frames painted in the default theme on every home crossing, Top 10 included when reached from home. `/` and every private path now share one route element, so the member's layout is one instance for the whole session. A visitor at `/` still gets the public landing; one anywhere else is still sent to `/login`.
