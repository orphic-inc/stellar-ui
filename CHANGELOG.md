...
- **A private page no longer flashes the default theme before the member's own** (#161). The pre-apply script already covered a return visit from the stored href, but with nothing stored the page painted in the default theme until the profile and stylesheet queries came back. That is a first-ever visit, and since #379 every first page after a login. `PrivateLayout` now holds its spinner until the theme's sheet has loaded, failed, or is not needed, for at most 3 s; past that it shows the page and keeps the link, so a slow theme still lands. ADR-0008's note that #161 was untouched is corrected.
